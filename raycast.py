...
class RayCast:
    ...
    def get_rendering_objects(self):
        ...
        for ray, values in enumerate(self.ray_cast_result):
            ...
            # clamp: pygame.transform.scale не принимает 0 или отрицательные
            ph = max(1, int(projection_height))

            if ph < HEIGHT:
                wall_column = self.textures[texture].subsurface(
                    int(offset * (TEXTURE_SIZE - SCALE)), 0, SCALE, TEXTURE_SIZE
                )
                wall_column = pygame.transform.scale(wall_column, (SCALE, ph))
                wall_column_position = (ray * SCALE, HALF_HEIGHT - ph // 2)
            else:
                texture_height = max(1, int(TEXTURE_SIZE * HEIGHT / ph))
                wall_column = self.textures[texture].subsurface(
                    int(offset * (TEXTURE_SIZE - SCALE)),
                    int(HALF_TEXTURE_SIZE - texture_height // 2),
                    SCALE,
                    texture_height
                )
    ...
    def update(self, render: bool = True):
        # render=False в RL/headless: только ray_cast (depths для obs),
        # get_rendering_objects не вызываем -> нет pygame.transform.scale
        self.ray_cast()
        if render:
